      ...
      task_1: [
        {
          questions: [
            {
              question:
                "Прочтите отрывок  из сочинения историка  С.М. Соловьева и ответьте на вопросы.",
      ...
      task_4: [
        {
          questions: [
            {
              question:
                "Какие из перечисленных дат связаны с деятельностью Александра Невского?",
              options: [
                { answer: "1242 г.", id: 1 },
                { answer: "1223 г.", id: 2 },
                { answer: "1252 г.", id: 3 },
    ...
    answer(questions) {
      ...
      for (let quest of questions) {
        for (let option of quest.options) {
          ...
          if (
            option.text.toLowerCase() === option.correct.toLowerCase() &&
            !option.answered
          ) {
...
<template>
  <div class="row-3">
    ...
    <div class="col">
      ...
      <div
        class="col frame fit row wrap justify-between items-center content-center"
      >
        <div class="filters">
    ...
    <q-card class="row test" v-for="task in task_1" :key="task.id">
      <div
        class="col-12 number fit row wrap justify-start items-start content-start"
      >
      ...
      <div class="text fit">
        <div
          ...
          v-for="quest in task.questions"
          :key="quest.id"
        >
          {{ quest.question }}
        </div>
        <div
          class="col-12 text-test"
          v-for="quest in task.questions"
          :key="quest.id"
        >
          {{ quest.text }}
      ...
      <div
        class="col-12 fit row wrap justify-between items-center content-center"
      >
        <div class="button">
          <q-btn class="answer-btn" @click="answer(task.questions)"
            >ответить
          </q-btn>
          <q-btn class="true-btn" @click="correct()"
            >показать правильные ответы</q-btn
          >
    ...
    <q-card class="row test" v-for="task in task_2" :key="task.id">
      ...
      >
        <div class="button">
          <q-btn class="answer-btn" @click="answer_2(task.questions)"
            >ответить</q-btn
          >
